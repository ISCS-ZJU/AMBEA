...
int main(int argc, char *argv[])
{
    ...
    int max_l = -1, max_r = 0, edges_num = 0;
    std::vector<std::vector<int> > neighbor_list;
    while(!fin.eof())
    {
        neighbor_list.push_back(getLine(fin, max_r));
        max_l ++;
        edges_num += neighbor_list[max_l].size();
    }
    std::cout<<max_l<<" "<<max_r<<" "<<edges_num<<std::endl;
    ...
    for(int i=0; i<neighbor_list.size(); i++)
    {
        ...
        new_neighbor_list.push_back(neighbor_list[i]);
    }
    for(int i=0, j=0; i<new_neighbor_list.size(); i++)
    {
    ...
    int new_max_l = 0, new_max_r = 0, new_edges_num = 0;
    ...
    std::cout<<new_max_l<<" "<<new_max_r<<" "<<new_edges_num<<std::endl;
    return 0;
}
